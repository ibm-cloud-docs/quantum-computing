...
- Familiarize yourself with the VQE program as described in the [sample programs](/docs/quantum-computing?topic=quantum-computing-sample-programs) topic.  
...
- Install the {{site.data.keyword.cloud_notm}} SDK: `pip install -U ibm-cloud-sdk-core`.
...
- Find your Cloud Resource Name (CRN). From the [{{site.data.keyword.cloud_notm}} console](https://cloud.ibm.com){: external}, scroll to Resource summary, click "Services and software", then click the row that contains your quantum service instance (not the name of the instance). In the pane that opens, click the icon to copy your CRN. For example:
   ...
   ```
Run the following command to create your authenticator. This keeps you from getting logged off when your IAM access token is renewed (hourly).

```python
from ibm_cloud_sdk_core.authenticators import IAMAuthenticator

authenticator = IAMAuthenticator('<insert-your-API-key-here>', url='https://iam.cloud.ibm.com')
...
response = requests.get(QISKIT_RUNTIME_ENDPOINT+'/programs', headers={'Authorization': 'Bearer '+authenticator.token_manager.get_token(), 'Content-Type': 'application/json', 'Service-CRN': CRN})
...
resp = requests.post(
    ...
    headers={'Authorization': 'Bearer '+authenticator.token_manager.get_token(), 'Content-Type': 'application/json', 'Service-CRN': CRN},
    data=inputs)
...
- [Available sample programs](/docs/quantum-computing?topic=quantum-computing-sample-programs)
